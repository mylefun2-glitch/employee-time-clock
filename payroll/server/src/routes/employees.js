...
router.put('/:id', validateId(), sanitizeBody, async (req, res) => {
  try {
    ...
    // Sync salary/allowances to Supabase
    if (salaryType !== undefined) {
      if (salaryType === 'hourly') {
        sbUpdate.hourly_rate = baseSalary !== undefined ? parseFloat(baseSalary) : existing.baseSalary;
        sbUpdate.base_salary = 0;
      } else {
        sbUpdate.base_salary = baseSalary !== undefined ? parseFloat(baseSalary) : existing.baseSalary;
        sbUpdate.hourly_rate = 0;
      }
    } else if (baseSalary !== undefined) {
      const currentSalaryType = salaryType || existing.salaryType;
      if (currentSalaryType === 'hourly') {
        sbUpdate.hourly_rate = parseFloat(baseSalary);
        sbUpdate.base_salary = 0;
      } else {
        sbUpdate.base_salary = parseFloat(baseSalary);
        sbUpdate.hourly_rate = 0;
      }
    }
    if (req.body.allowanceManager !== undefined) sbUpdate.allowance_manager = parseFloat(req.body.allowanceManager);
    if (req.body.allowanceLicense !== undefined) sbUpdate.allowance_license = parseFloat(req.body.allowanceLicense);
    if (req.body.otherAllowance !== undefined) sbUpdate.other_allowance = parseFloat(req.body.otherAllowance);
    ...
    const numericFields = [
      ...
      'allowanceAA', 'allowanceLicense', 'allowanceManager',
      'laborInsuranceGrade', 'healthInsuranceGrade', 'laborPensionGrade',
