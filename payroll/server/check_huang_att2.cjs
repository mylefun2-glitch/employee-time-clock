...
async function check() {
  const { data: logs, error } = await supabase.from('attendance_logs')
    .select('*')
    .eq('employee_id', '9980cfaf-b375-4db6-8338-91a98ad5b6e8')
    .gte('timestamp', '2026-05-01')
    .lte('timestamp', '2026-05-31');
  console.log(`Supabase attendance_logs count: ${logs ? logs.length : 0}`, error || '');
}
