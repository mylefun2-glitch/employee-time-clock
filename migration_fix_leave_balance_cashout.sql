-- Migration: Fix Leaf Balance Cashout (ALC & CO)
-- Date: 2026-03-17

-- Update get_employee_leave_balances with proper ALC and CO cashout summation
...
DECLARE
    ...
    alc_cashout_hours decimal(10, 2) := 0;
...
BEGIN
    ...
    SELECT COALESCE(SUM(hours), 0) INTO alc_cashout_hours FROM leave_requests
    WHERE employee_id = target_employee_id AND status = 'APPROVED' AND (is_modified IS FALSE OR is_modified IS NULL)
    ...
    total_annual_cashout := total_annual_cashout + COALESCE(alc_cashout_hours, 0);
    ...
    DECLARE
        ...
        co_cashout_hours decimal(10, 2) := 0;
    BEGIN
        ...
        SELECT COALESCE(SUM(hours), 0) INTO co_cashout_hours FROM leave_requests
        WHERE employee_id = target_employee_id AND status = 'APPROVED' AND (is_modified IS FALSE OR is_modified IS NULL)
        ...
        total_comp_cashout := total_comp_cashout + COALESCE(co_cashout_hours, 0);
