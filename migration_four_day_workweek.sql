-- Migration: Add Four-Day Workweek Periods and Proportional Special Leave Calculation
-- Date: 2026-03-15

-- 1. Create table for four-day workweek periods
CREATE TABLE IF NOT EXISTS four_day_workweek_periods (
...
-- Enable RLS
ALTER TABLE four_day_workweek_periods ENABLE ROW LEVEL SECURITY;

-- Policies
DO $$ BEGIN
    DROP POLICY IF EXISTS "Enable all access for all users" ON four_day_workweek_periods;
EXCEPTION WHEN OTHERS THEN END $$;

CREATE POLICY "Enable all access for all users" ON four_day_workweek_periods FOR ALL USING (true) WITH CHECK (true);

-- 2. Create helper function to calculate the four-day workweek ratio and formula text
...
BEGIN
    ...
    IF four_day_days > 0 THEN
        ...
        detail_text := '(正常 ' || normal_days || '天+週休三日 ' || four_day_days || '天*0.8)/' || total_days || '天';
    ELSE
        ratio := 1.0;
        detail_text := '';
    END IF;
...
-- 3. Update get_employee_leave_balances with four-day workweek proportional deduction
...
DECLARE
    ...
    p_ratio decimal; p_ratio_text text;
...
BEGIN
    ...
    IF (adjusted_join_dt + interval '6 months') < '2017-01-01'::date AND (adjusted_join_dt + interval '1 year') > '2017-01-01'::date THEN
        ...
        SELECT ratio, detail_text INTO p_ratio, p_ratio_text FROM calculate_four_day_workweek_ratio(target_employee_id, adjusted_join_dt, p_start);
        ...
        bucket_list := bucket_list || jsonb_build_object(
            'label', '滿 0.5 年 (新制轉銜)', 'start_date', p_start, 'end_date', p_end, 'entitlement', ROUND(3 * p_ratio * p_weighted_avg_hours, 2),
            'formula', CASE WHEN p_ratio < 1.0 THEN '3天 * [' || p_ratio_text || '] * ' || p_weighted_avg_hours || '小時' ELSE '3天 * ' || p_weighted_avg_hours || '小時' END,
            'detail_formula', CASE WHEN p_ratio < 1.0 THEN '3天 * [' || p_ratio_text || '] * ' || p_weighted_avg_hours || '小時 (2017新制轉銜補貼)' ELSE '3天 * ' || p_weighted_avg_hours || '小時 (2017新制轉銜補貼)' END,
    ...
    IF (adjusted_join_dt + interval '6 months') >= '2017-01-01'::date AND target_date >= (adjusted_join_dt + interval '6 months') THEN
        ...
        bucket_list := bucket_list || jsonb_build_object(
            'label', '滿 0.5 年', 'start_date', p_start, 'entitlement', ROUND(3 * p_ratio * p_weighted_avg_hours, 2),
            'formula', CASE WHEN p_ratio < 1.0 THEN '3天 * [' || p_ratio_text || '] * ' || p_weighted_avg_hours || '小時' ELSE '3天 * ' || p_weighted_avg_hours || '小時' END,
            'detail_formula', CASE WHEN p_ratio < 1.0 THEN '3天 * [' || p_ratio_text || '] * ' || p_weighted_avg_hours || '小時 (依年資 ' || (adjusted_join_dt::date) || '~' || (p_start::date) || ' 加權工時)' ELSE '3天 * ' || p_weighted_avg_hours || '小時 (依年資 ' || (adjusted_join_dt::date) || '~' || (p_start::date) || ' 加權工時)' END,
            'date_formula', p_date_formula
        );
    END IF;
    ...
    FOR i IN 1..25 LOOP
        ...
        SELECT ratio, detail_text INTO p_ratio, p_ratio_text FROM calculate_four_day_workweek_ratio(target_employee_id, p_earning_start, p_start);
        
        SELECT COALESCE(SUM(end_date - start_date + 1), 0) INTO p_susp_days FROM seniority_suspensions WHERE employee_id = target_employee_id AND start_date < p_start;
        p_date_formula := CASE WHEN p_susp_days > 0 THEN '原 ' || (join_dt + (i * interval '1 year'))::date || ' + ' || p_susp_days || '天' ELSE NULL END;

        bucket_list := bucket_list || jsonb_build_object(
            'label', '滿 ' || i || ' 年', 'start_date', p_start, 'entitlement', ROUND(p_days * p_ratio * p_weighted_avg_hours, 2),
            'formula', CASE WHEN p_ratio < 1.0 THEN p_days || '天 * [' || p_ratio_text || '] * ' || p_weighted_avg_hours || '小時' ELSE p_days || '天 * ' || p_weighted_avg_hours || '小時' END,
            'detail_formula', CASE WHEN p_ratio < 1.0 THEN p_days || '天 * [' || p_ratio_text || '] * ' || p_weighted_avg_hours || '小時 (依年資 ' || (p_earning_start::date) || '~' || (p_start::date) || ' 加權工時)' ELSE p_days || '天 * ' || p_weighted_avg_hours || '小時 (依年資 ' || (p_earning_start::date) || '~' || (p_start::date) || ' 加權工時)' END,
            'date_formula', p_date_formula
        );
    END LOOP;
    ...
    DECLARE
        ...
        ot_type_id uuid; toil_type_id uuid; comp_leave_type_id uuid;
    ...
    BEGIN
        ...
        -- Use the same logic for comp leave as in update_leave_balance_function.sql
        ...
        WHERE employee_id = target_employee_id AND leave_type_code IN ('TOIL', 'COMPENSATORY') AND adjustment_type = 'CASHOUT';
        ...
        FOR i IN 0..full_years LOOP
            ...
            WHERE employee_id = target_employee_id AND leave_type_code IN ('TOIL', 'COMPENSATORY') AND adjustment_type IN ('GRANT', 'CORRECTION')
