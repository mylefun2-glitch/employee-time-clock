...
const ResourceCalendarPage: React.FC = () => {
    ...
    const fetchData = async () => {
        setLoading(true);
        // Date range filtering can be added to the service later, but for now we fetch APPROVED/PENDING
        try {
                ...
                <div className="flex flex-row items-center justify-between w-full gap-4">
                    ...
                    <div className="flex items-center gap-2 print:hidden">
                        ...
                        <button
                            ...
                            className="inline-flex items-center px-4 py-2 bg-violet-600 text-white rounded-xl text-xs font-black hover:bg-violet-700 transition-all shadow-md shadow-violet-100"
                        >
                            <span className="material-symbols-outlined text-[16px] mr-1">handshake</span>
                            <span className="hidden sm:inline">借用申請</span>
                        </button>
                    </div>
                </div>
            </div>
