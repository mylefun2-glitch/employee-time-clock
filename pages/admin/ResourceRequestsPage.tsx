...
const ResourceRequestsPage: React.FC = () => {
    ...
    const handleReview = async (status: 'APPROVED' | 'REJECTED') => {
        ...
        try {
            // Find admin employee id — use user.id as approver reference
    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div>
                <h1 className="text-2xl font-black text-slate-900 tracking-tight">借用申請審核</h1>
                <p className="text-slate-500 text-sm font-medium mt-1">審核員工的物品及場地借用申請</p>
            </div>
                                ...
                                <div className="flex items-center gap-4 min-w-0">
                                    ...
                                    <div className="min-w-0">
                                        ...
                                        <div className="text-sm font-bold text-slate-700 mt-1">
                                            ...
                                            {req.quantity > 1 && <span className="text-xs text-slate-400 ml-1">× {req.quantity}</span>}
                                        </div>
                                        <div className="text-xs text-slate-400 font-medium mt-0.5">
                                            {formatDateTime(req.start_time)} – {formatDateTime(req.end_time)}
                                        </div>
                                        <div className="text-xs text-slate-500 mt-0.5">用途：{req.purpose}</div>
                                        {req.review_comment && (
                                            <div className="text-xs text-slate-400 mt-0.5 italic">備註：{req.review_comment}</div>
                                        )}
                                    </div>
                                </div>
                                {req.status === 'PENDING' && (
                                    <button
                                        ...
                                        className="flex items-center gap-2 px-5 py-2.5 bg-violet-600 text-white rounded-xl font-black text-sm shadow-lg shadow-violet-100 hover:bg-violet-700 transition-all shrink-0"
                                    >
            ...
            {reviewingRequest && (
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-in fade-in duration-300">
                    <div className="bg-white rounded-[2.5rem] shadow-2xl max-w-md w-full p-8 animate-in zoom-in-95 duration-300">
                        ...
                        <div className="bg-slate-50 rounded-2xl p-4 mb-6 text-sm space-y-1">
                            ...
                            <div className="flex gap-2">
                                <span className="text-slate-400 font-bold min-w-[56px]">數量</span>
                                <span className="text-slate-700 font-bold">{reviewingRequest.quantity}</span>
                            </div>
                            <div className="flex gap-2">
