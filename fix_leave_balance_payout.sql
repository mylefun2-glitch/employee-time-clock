-- Fix leave balance calculation for CO (Overtime Conversion) and ALC (Special Leave Payout)
-- Date: 2026-04-15
    ...
    BEGIN
        ...
        IF full_years >= 1 THEN
            FOR i IN REVERSE full_years..1 LOOP
                ...
                -- Cashout (Adjustments + ALC requests)
                ...
                DECLARE
                    req_alc_hours decimal(10, 2);
                BEGIN
                    SELECT COALESCE(SUM(hours), 0) INTO req_alc_hours FROM leave_requests
                    WHERE employee_id = target_employee_id AND status = 'APPROVED' AND (is_modified IS FALSE OR is_modified IS NULL)
                      AND leave_type_id = alc_type_id
                      AND start_date >= p_start AND start_date < p_end;
                    p_cashout_hours := p_cashout_hours + req_alc_hours;
                END;
        ...
        -- Manual annual adjustments (GRANT/CORRECTION)
        DECLARE
        ...
        BEGIN
            ...
            FOR i IN REVERSE periods_count..1 LOOP
                ...
                -- Cashout (Adjustments + CO requests)
                ...
                DECLARE
                    req_co_hours decimal(10, 2);
                BEGIN
                    SELECT COALESCE(SUM(hours), 0) INTO req_co_hours FROM leave_requests
                    WHERE employee_id = target_employee_id AND status = 'APPROVED' AND (is_modified IS FALSE OR is_modified IS NULL)
                      AND leave_type_id = co_type_id
                      AND start_date >= p_start AND start_date < p_end;
                    p_comp_cashout := p_comp_cashout + req_co_hours;
                END;
            ...
            -- Total Overtime (Real OT only, for reference)
